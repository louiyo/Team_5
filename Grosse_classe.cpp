...
void write_seq(std::ofstream&, bool forward=true);
								...
								if(line[0] == '>')
									{ 	
										...
										if(chromo_seq.size() != 0)
										{
											...
											while(Sequence.getPosition() < chromo_seq.size() )
											...
											extract_seq(chromo_seq, Matrice.getSeqLength());
											
											//on discrimine si la séquence et ses paramètres peuvent être écrits dans le fichier de sortie
											if(Matrice.reverse_score(Sequence.getSequence))
											{
												write_seq(file_output, false);
											}
											
											if(Matrice.forward_score(Sequence.getSequence))
												{
										...
										Sequence.change_chromosome(chromo_name);
